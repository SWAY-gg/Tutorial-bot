...
# Kick
    ...
    async def mod_kick(self, ctx, user: discord.Member = None, *, reason = "Отсуствует"):
        ...
        if user == ctx.author:                              return
        if user.top_role >= ctx.author.top_role:            return
...
# HackBan
    ...
    async def hackban_command(self, ctx, userid = None, *, reason = 'Отсуствует'):
        if userid == None: return
        if len(userid) > 18: return
        if (len(reason) > 401): return
